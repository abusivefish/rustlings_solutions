...
fn read_and_validate(b: &mut dyn io::BufRead) -> Result<PositiveNonzeroInteger, Box<dyn error::Error>> { // added Box<> return type(s)
    let mut line = String::new();
    b.read_line(&mut line)?; //add ? -- not sure I understand -- read_line()? probably handles empty strings in addition to a valid value. 
    let num: i64 = line.trim().parse()?; // add ?
    let answer = PositiveNonzeroInteger::new(num)?; // add ?
    Ok(answer) // statically return Ok condition provided we didn't crash
}
